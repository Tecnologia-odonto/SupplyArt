/*
  ...
  2. Security
    ...
    - Op. Almoxarife: access to quotations from their CD
    - Op. Financeiro: read-only access to all quotations
...
CREATE TABLE IF NOT EXISTS quotations (
  ...
  CONSTRAINT quotations_status_check CHECK (status IN ('rascunho', 'enviada', 'em_analise', 'finalizada', 'cancelada'))
);
...
CREATE POLICY "Financial operators can read quotation items"
  ON quotation_items
  FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY "Financial operators can read quotation responses"
  ON quotation_responses
  FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY "Users can manage price history"
  ON price_history
  FOR ALL
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM profiles 
      WHERE profiles.id = auth.uid() 
      AND profiles.role IN ('admin', 'gestor', 'operador-almoxarife', 'operador-financeiro')
    )
  )
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM profiles 
      WHERE profiles.id = auth.uid() 
      AND profiles.role IN ('admin', 'gestor', 'operador-almoxarife')
    )
  );
